...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                        style={'textAlign': 'center', 'color': '#503D36',
                                               'font-size': 40}),
                                dcc.Dropdown(id='site-dropdown',  # ID for the dropdown component
                                             options=[
...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        ...
        # Filter the data for the selected launch site
        site_df = spacex_df[spacex_df['Launch Site'] == selected_site]
        # Create a pie chart for the selected launch site
        fig = px.pie(site_df, names='class', title=f'Success vs. Failed Launches for {selected_site}')
    return fig
...
def update_scatter_chart(selected_site, payload_range):
    ...
    # Create a scatter chart to show the correlation
